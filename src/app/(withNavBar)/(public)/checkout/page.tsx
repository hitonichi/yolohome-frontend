      ...
      <Form {...form}>
        <form onSubmit={handleSubmit(onSubmit)} className="w-full flex justify-start items-start px-20 py-6 gap-20">
          <div className="grid grid-cols-6 w-full gap-x-8 gap-y-4 justify-start items-start">
            ...
            />
            <Label className="font-semibold col-span-6 mt-4">Payment Method</Label>
            <RadioGroup
              onValueChange={(val) => setPaymentMethod(val)}
              defaultValue={'COD'}
              className="col-span-6 pl-1 flex items-center just-between gap-8"
            >
              <div className="flex items-center justify-start gap-2">
                <RadioGroupItem value={'COD'} id="COD" />
                <Label className="text-sm font-semibold  cursor-pointer" htmlFor="COD">
                  COD
                </Label>
              </div>
              <div className="flex items-center justify-start gap-2">
                <RadioGroupItem value={'Paypal'} id="Paypal" />
                <Label className="text-sm font-semibold  cursor-pointer" htmlFor="Paypal">
                  PayPal
                </Label>
              </div>
            </RadioGroup>
          ...
          <Card className="w-96 min-w-[30%]">
            ...
            <CardContent>
              <div className="grid grid-cols-2 justify-between">
                ...
                <Link href={'/checkout'} className={`col-span-2 w-full mt-4 ${paymentMethod != 'COD' ? 'hidden' : ''}`}>
                  <Button className="w-full">Checkout</Button>
                </Link>
                <div className={`mt-4 w-full col-span-2 ${paymentMethod != 'Paypal' ? 'hidden' : ''}`}>
